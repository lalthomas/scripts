...
case  $dayOfWeeK  in
      "Monday") 	
      	...
      	filename=$today-"monday work journal"$extension
      	;;
      "Tuesday")	
      	filetocopy="/planner-02-tuesday.md"
      	filename=$today-"tuesday work journal"$extension
      	;;            
      	 
      "Wednesday")	
      	filetocopy="/planner-03-wednesday.md"
      	filename=$today-"wednesday work journal"$extension
      	;;
      "Thursday") 	
      	filetocopy="/planner-04-thursday.md"
      	filename=$today-"thursday work journal"$extension
      	;;
      "Friday") 	
      	filetocopy="/planner-05-friday.md"
      	filename=$today-"friday work journal"$extension
      	;;
      "Saturday")	
      	filetocopy="/planner-06-saturday.md"
      	filename=$today-"saturday work journal"$extension
      	;;
      "Sunday") 	
      	filetocopy="/planner-07-sunday.md"
      	filename=$today-"sunday work journal"$extension
      	;;
      *)              
esac 
...
if [ ! -f "$COPYDIR"/"$filename" ];then
  ...
  # Read input file into a string variable. 
  # Thanks : http://stackoverflow.com/a/2789399/2182047
  copyfilecontent=$(cat $BASEDIR"/planner-daily.md")
  #copy contents to journal file
  echo "$copyfilecontent" >>"$journalfilepath"
